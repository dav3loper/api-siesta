...
class ObtainMoviesFromLetterboxdCommand extends Command
{
    ...
    public function __construct(
        ...
        private readonly SynopsisFinder $synopsisFinder,
    )
    {
        parent::__construct();
    }
    ...
    protected function configure(): void
    {
        ...
        $this->setDescription('Seed movies from a Letterboxd list (title + trailer + TMDB poster + TMDB synopsis, translated to Spanish via DeepL when TMDB has no Spanish overview; duration/sessions are left empty and get filled in by re-running obtain-movies with the official JSON once it is available)')
    ...
    public function execute(InputInterface $input, OutputInterface $output): int
    {
        ...
        foreach ($entries as $index => $entry) {
            ...
            $this->movieRepository->store(new Movie(
                ...
                $trailer,
                0,
                $this->synopsisFinder->findByTitle($entry->title, $entry->year),
                $entry->link,
                $editionId,
                '',
                [],
